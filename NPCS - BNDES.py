...
def simular(mu, num_funcionarios, num_anos, conservador):
    ...
    return mu, porcentagem_teto_salarial, salario_bruto_mensal_from_anual(novos_salarios_anual_exercicios_media), promocoes_acumuladas_exercicios, conservador

# Executar simulação
mu, porcentagem_teto_salarial, salarios_mensal_exercicios_media, promocoes_acumuladas_exercicios, conservador = simular(mu=3.0, num_funcionarios=NUM_FUNCIONARIOS, num_anos=NUM_ANOS, conservador=True)

if conservador:
    legenda = '(Hipótese Conservadora)'
else:
    legenda = '(Hipótese Otimista)'
...
plt.title(f'Distribuição de Poisson Truncada {legenda}', fontsize=16)
...
plt.savefig(f"Distribuição de Poisson Truncada {legenda}.png", format="png")
...
ax.set_ylim(0, 110)
ax.set_title(f'Porcentagem de Funcionários que Atingiram o Topo da Carreira ao Longo dos Anos {legenda}', fontsize=16)
...
# Salvar a figura
plt.savefig(f"Porcentagem de Funcionários que Atingiram o Topo da Carreira ao Longo dos Anos {legenda}.png", format="png", bbox_inches='tight')
...
ax.set_title(f'Salário Líquido Médio Anualizado dos Funcionários ao Longo dos Anos {legenda}', fontsize=16)
ax.set_xlabel('Ano', fontsize=14)
...
# Chamar a função para adicionar legendas
adicionar_legendas_barras(ax, espacamento_legendas=3, monetario=True)

# Salvar a figura
plt.savefig(f"Salário Bruto Médio dos Funcionários ao Longo dos Anos {legenda}.png", format="png", bbox_inches='tight')
...
ax.set_title(f'Número de Steps Médio Concedidos ao Longo dos Anos {legenda}', fontsize=16)
ax.set_xlabel('Ano', fontsize=14)
...
# Salvar a figura
plt.savefig(f"Número de Steps Médio Concedidos ao Longo dos Anos {legenda}.png", format="png", bbox_inches='tight')
